     ...
     for($i=1;$i<=$Divisions+1;$i++)
      {
       ...
       if ( $DataDescription["Format"]["Y"] == "number" )
        $Value = round($Value.$DataDescription["Unit"]["Y"]);
       ...
       if ( $RightScale )
       ...
       else
        {
         ...
         imagettftext($this->Picture,$this->FontSize,0,$this->GArea_X1-10-$TextWidth,$YPos+($this->FontSize/2),$C_TextColor,$this->FontName,round($Value));
...
//           if ($ID % 2 == 0)
   ...
   // for wildfire draw x and y axis legend
   function drawAxisLegend($XPos,$YPos,$Value,$R,$G,$B,$Rs=-1,$Gs=-1,$Bs=-1,$Rt=0,$Gt=0,$Bt=0,$Border=TRUE)
    {
     $C_TextColor =$this->AllocateColor($this->Picture,$Rt,$Gt,$Bt);
     ...
     imagettftext($this->Picture,$this->FontSize,0,$XPos+8,$YPos+$YOffset,$C_TextColor,$this->FontName,$Value);
     $Position   = imageftbbox($this->FontSize,0,$this->FontName,$Value);
    }
       ...
       foreach ( $Data as $Key => $Values )
        {
         ...
         $XPos = $XPos + $this->DivisionWidth;
         if (count($DataDescription["Values"]) == 1
            && count($Data) < 10) { //for wildfire 
             if(++$ColorID >= count($this->Palette)) {
                 $ColorID = 0;
             }
         }
        }
